...
$$\mathbb{V}\mathrm{ar}(\hat{\mathbf{\beta}}(\lambda) ) = \sigma^2 \mathbf{W}_\lambda (\mathbf{X}^\mathsf{T}\mathbf{X} )^{-1}\mathbf{W}^\mathsf{T}_\lambda$$ where $\mathbf{W}_\lambda = [\mathbf{I}_{p\times p} + \lambda(\mathbf{Z}^\mathsf{T}\mathbf{X})^{-1}]^{-1}$ and the irreducible error $\sigma^2$ is set to $\sigma^2=1$.
