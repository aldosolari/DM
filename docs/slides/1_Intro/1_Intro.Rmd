...
* Suppose that yesterday we observed $n = 30$ pairs of data, the __training set__
...
* We wish to obtain an estimate of $f(x)$ that allows us to predict future $y$ as new observations of $x$ become available
...
these new $y^*_i$ are associated with the same $x_i$ used for yesterday's data
...
```{r, eval = T, message=F, warning=F, error=F, comment=NA}
library(readr)
...
* Suppose we have estimated $f(x)$ by $\hat{f}(x)$
...
* Find the degree $d$ that minimizes $\mathbb{E}(\mathrm{MSE}_{\mathrm{Te}})$
