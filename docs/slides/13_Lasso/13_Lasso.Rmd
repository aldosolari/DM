...
* Since variable selection is part of the model building process,  cross-validation should account for the variability of the selection when calculating performance

* To get performance estimates that incorporate the variation due to variable selection, it is wiser to perform subset selection inside an outer layer of K-fold cross-validation

* Note that possibly different subsets of the "best" predictors are generated at each iteration
