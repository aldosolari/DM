...
In the Fixed-X setting we have the training set 
$$(x_1,y_1),\ldots,(x_n,y_n)$$ 
and the test set 
$$(x_1,y^*_1),\ldots,(x_n,y^*_n)$$
where 

* the training values $x_1,\ldots,x_n$ are treated as fixed (i.e. non-random)
* the test values exactly match the the training values 

The responses $y_1, \ldots, y_n$ are realizations of the random variables
$$Y_i = f(x_i)+ \varepsilon_i, \quad i=1,\ldots,n$$
and $y^*_1, \ldots, y^*_n$ are realizations of the random variables
$$Y^*_{i}=f(x_i)+\varepsilon^*_i, \quad i=1,\ldots,n$$
where $f$ is called the __regression function__ and 
$\varepsilon_1,\ldots,\varepsilon_n$ and $\varepsilon^*_1,\ldots,\varepsilon^*_n$ are i.i.d. with $\mathbb{E}(\varepsilon)=0$ and $\mathbb{V}\mathrm{ar}(\varepsilon)=\sigma^2$. 
...
\cdots\\
y_n \\
\end{array}\right], \qquad \underset{n\times 1}{\mathbf{y}^*} = 
\left[
\begin{array}{c}
y^*_1   \\
\cdots\\
...
In the fixed-X setting, the __prediction error__ $\mathrm{ErrF}$ is defined by
...
This is the error that we wish to minimize.
...
$$\mathrm{ErrF} =  \sigma^2 + \underbrace{\frac{1}{n}\sum_{i=1}^{n}(\mathbb{E}\hat{f}(x_i) - f(x_i) )^2}_{\mathrm{Bias}^2} + \underbrace{\frac{1}{n}\sum_{i=1}^{n} \mathbb{V}\mathrm{ar}(\hat{f}(x_i))}_{\mathrm{Variance}} = \sigma^2 + [\mathrm{Bias}(\hat{f})]^2+\mathrm{Variance}(\hat{f})$$
