...
```{r}
fit = glmnet(Xraw, yraw, alpha = 0, family="gaussian")
# solution path
plot(fit, xvar="lambda")
```
...
* In practice, it is unknown and the user needs to make an
informed decision on its value
...
K <- 5
fit.cv <-cv.glmnet(Xraw,yraw,alpha=0, nfolds = K)
plot(fit.cv)
```
...
$$\mathbb{E}(\hat{\boldsymbol{\beta}}^{\lambda}) =  \mathbf{X}^\mathsf{T}\mathbf{X}(\lambda\mathbf{I}_{p\times p}  + \mathbf{X}^\mathsf{T}\mathbf{X})^{-1} \boldsymbol{\beta}$$

* The linear operator 
$$\mathbf{W}^{\lambda} = [\mathbf{I}_{p\times p}+ \lambda(\mathbf{X}^\mathsf{T}\mathbf{X})^{-1}]^{-1}$$
transforms the OLS estimator into the ridge estimator $$\mathbf{W}^{\lambda} \hat{\boldsymbol{\beta}} = \hat{\boldsymbol{\beta}}^{\lambda}$$

The variance of the ridge estimator
$$\mathbb{V}\mathrm{ar}(\hat{\boldsymbol{\beta}}^{\lambda}) =  \mathbf{W}^{\lambda}  \mathbb{V}\mathrm{ar}(\hat{\boldsymbol{\beta}}) (\mathbf{W}^{\lambda})^\mathsf{T} = \sigma^2 \mathbf{W}^{\lambda} (\mathbf{X}^\mathsf{T}\mathbf{X} )^{-1}(\mathbf{W}^{\lambda})^\mathsf{T}$$

---

# Exercize: prostate cancer data
...
The response variable is lpsa, and the predictor variables are lcavol, lweight, age, lbph, svi, lcp, gleason, pgg45
...
* Compute the rigde estimate
$$\hat{\boldsymbol{\beta}}^\lambda = (\mathbf{X}^\mathsf{T}\mathbf{X} + \lambda \mathbf{I}_{p})^{-1}\mathbf{X}^\mathsf{T}\mathbf{y}$$
for $\lambda=10$.
...
* For $\lambda = 0, 1, 2,\ldots ,9,10$, plot the solution path of the estimated coeffients $\hat{\beta}_1^\lambda,\ldots,\hat{\beta}_p^\lambda$ and the corresponding variances $\mathbb{V}\mathrm{ar}(\hat{\beta}_1^\lambda), \ldots, \mathbb{V}\mathrm{ar}(\hat{\beta}_p^\lambda)$, which are the elements in the diagonal of the variance/covariance matrix
$$\mathbb{V}\mathrm{ar}(\hat{\boldsymbol{\beta}}^\lambda) = \sigma^2 \mathbf{W}_\lambda (\mathbf{X}^\mathsf{T}\mathbf{X} )^{-1}\mathbf{W}^\mathsf{T}_\lambda$$ where $\mathbf{W}_\lambda = [\mathbf{I}_{p\times p} + \lambda(\mathbf{X}^\mathsf{T}\mathbf{X})^{-1}]^{-1}$ and is set to $\sigma^2=1$.
...
* Compare the LOOCV estimate and the test MSE for the linear model and the ridge regression model with $\lambda=5$. 
