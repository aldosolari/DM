...
Optimism is an interesting and important concept because an estimate of $\widehat{\mathrm{OptF}}$ of $\mathrm{OptF}$ leads to an estimate of prediction error $\mathrm{ErrF} = \mathbb{E}(\mathrm{MSE}_{\mathrm{Te}}) = \mathbb{E}(\mathrm{MSE}_{\mathrm{Tr}}) + \mathrm{OptF}$:
\begin{aligned}
...
Because we don't know $\sigma^2$, we can plug-in its estimate $$\hat{\sigma}^2= \frac{\mathrm{RSS}}{n-p}= \frac{\sum_{i=1}^{n}( y_i - \hat{f}(x_i))^2}{n-p}$$ where $\mathrm{RSS} = n \mathrm{MSE}_{\mathrm{Tr}}$ is the __residual sum of squares__. This gives
\begin{aligned}
...
We illustrate the optimism with the yesterday-tomorrow data: 
...
In the last lecture we have computed $\mathrm{MSE}_{\mathrm{Tr}}$ and $\mathrm{ErrF}$. Now we compare these values to $\widehat{\mathrm{ErrF}} = \mathrm{MSE}_{\mathrm{Tr}}  + \frac{2\sigma^2 p}{n}$ knowing that the true $\sigma=0.01:
