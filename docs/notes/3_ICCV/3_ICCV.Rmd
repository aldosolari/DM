...
its statistical performance on the same data yields an overoptimistic result. __Cross-validation__ (CV) was raised to fix this issue, starting from the remark that testing the output of the algorithm on new data would yield a good estimate $\widehat{\mathrm{Err}}$ of the expected test error $\mathbb{E}(\mathrm{MSE}_{\mathrm{Te}})$
...
heuristics. Therefore, CV is a non-parametric method which can be applied to any algorithm in any framework. This universality is not shared by e.g. Cp, which is specific to linear regression.
