...
Up to now, we have only dealt with parametric methods, but it is also worth exploring __nonparametric__ ones. Non-parametric methods do not make explicit assumptions about the functional
form of $f$ (e.g. polynomials). This means to leave data `speak for themselves' in a free way.

* Such approaches
can have a major advantage over parametric approaches: by avoiding the assumption of a particular functional form for $f$, they have the potential to accurately fit a wider range of possible shapes for $f$.
...
* [Fig. 2.7 ISLR](http://www-bcf.usc.edu/~gareth/ISL/Chapter2/2.7.pdf)

# k-nearest neighbors

A very simple and quite commonly used method is __k-nearest neighbors__ (kNN). The estimate of the
function $f$ at each point is obtained as the average of a fixed number of closest observations around that point. This method is called $k$–nearest–neighbors, where
$k$ denotes the number of observations averaged by the estimate.

Suppose we want to make a prediction at some $x_0$. Define the neighbourhood $N_k(x_0)$ to be the set of $k$ training observations having values $x_i$ closest to $x_0$ in Euclidean norm $\| x_i - x_0 \|=\sqrt{(x_i - x_0)^\mathsf{T}(x_i - x_0)}$

$$\hat{f}(x_0) = \frac{1}{k}\sum_{i \in N_k(x_0)} y_i$$

As in the case of regression, the number $k$ is a __tuning parameter__ related to the complexity of the model. By varying the number of neighbours $k$, we can achieve a wide range of flexibility in the estimated function. 

* Small $k$ corresponds to a more flexible fit: the $k$ points are closer to target $x_0$ (low bias), but averages based on a small sample have high variance. 
* Large k corresponds to a less flexible fit: it includes points
far from $x_0$ (high bias), but have smaller variance.
* Ordinarily, since we are computing a distance, we would want to __center__ and __scale__ the predictors.

The best value for $k$ depends on how smooth the true function $f$ is, and how noisy $y$ is. We can try different values of $k$ and use e.g. cross-validation to select one value. 

Note that kNN is a special case of __local regression__ of degree 0, when a nonconstant bandwidth along the x-axis is chosen. 

# Exercise

Do the following exercize. Consider the $n=250$ points version of the yesterday-tomorrow data:
...
1. Obtain the training set and test set by
...
2. Install the R package `kknn` and perform kNN with $k=21$ by using the function 
...
3. Compute MSE.tr and MSE.te for $k=21$

```{r, echo=F}
MSE.tr = mean( (train$y - yhat)^2 )
print(paste("kNN with k =",k, "MSE.tr =", MSE.tr))
MSE.te = mean( (test$y - yhat)^2 )
print(paste("kNN with k =",k, "MSE.te =", MSE.te))
```

4. Select the tuning parameter $k$ via leave-one-out by the function `train.kknn(y ~ x, train, ks = 1:40, kernel = "rectangular")`

```{r, echo=F}
ks = 1:40
LOOCV = train.kknn(y ~ x, train, ks = ks, kernel = "rectangular")$ best.parameters$k
print(paste("Best k with LOOCV =", LOOCV))
```

5. Compute MSE.tr and MSE.te for $k=1,2,\ldots,40$ and find the $k$ corresponding to the minimum MSE.te
...
6. For the fixed-X setting, theoretical results show that for kNN
...
7. In the fixed-X setting, theoretical results show that for kNN
...
The package utilizes a number of R packages but tries not to load them all at package start-up. The package "suggests" field includes 76 packages. `caret` loads packages as needed and assumes that they are installed.
Install caret using
```{r, eval=F}
 install.packages("caret", dependencies = c("Depends", "Suggests"))
```
to ensure that __all__ the needed packages are installed.
...
* To change the candidate values of the tuning parameter, either of the `tuneLength` or `tuneGrid` arguments can be used. The train function can generate a candidate set of parameter values and the tuneLength argument controls how many are evaluated. The `tuneGrid` argument is used when specific values are desired.
...
Compare to polynomial regression of degree $d=6$ (which is the best choice for this example)
...
Now consider a model building process that uses the training data to build a prediction function $\hat{f}$. We can associate to this procedure the __Random-X prediction error__
...
Suppose that $\mu_x=\mu_y=0$, $\rho=0.5$, $\sigma_x=1$, $\sigma_y=2$ thus $X\sim N(0,1)$, $\alpha=0$, $\beta=1$, $\sigma^2=3$ and $Y|X=x \sim N(\beta x,\sigma^2)$.
