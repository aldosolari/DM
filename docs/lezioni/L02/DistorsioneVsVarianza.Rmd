...
# Varianza per il modello lineare
...
dove $\underset{n \times p}{\mathbf{X}}$ è la matrice del disegno, con matrice di varianza delle stima 
$$\mathbb{V}\mathrm{ar}(\hat{\beta})=\sigma^2 (\mathbf{X}^\mathsf{T} \mathbf{X})^{-1}$$

* Quindi la varianza della previsione $\hat{f}(x_i)=x_i^\mathsf{T}\hat{\beta}$ (dove $x_i^\mathsf{T}$ è l' $i$-sima riga di $\mathbf{X}$) risulta pari a
...
# distorsione: per ottenere il valore atteso dello stimatore, stimo il modello senza errori
