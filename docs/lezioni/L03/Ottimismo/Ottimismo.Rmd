---
title: "Data Mining"
subtitle: "Ottimismo e criteri di informazione"
author: Aldo Solari
output:
...
che dobbiamo operare un compromesso tra componente di 
distorsione e di varianza

* Operativamente però non possiamo utilizzare la conoscenza di $f(x)$, ovviamente ignota in pratica

* Abbiamo visto che la trappola da evitare è il sovra-adattamento ai dati. Presenteremo due approcci: 
...
L'ottimismo è definito come differenza attesa tra l'errori del test e del training
...
* Nel setting Fixed-X, questo corrisponde a

$$\mathrm{OptF} = \mathbb{E}\left[ \frac{1}{n}\sum_{i=1}^{n}( y^*_i - \hat{f}(x_i))^2 - \frac{1}{n}\sum_{i=1}^{n}( y_i - \hat{f}(x_i))^2 \right]$$

* $\mathrm{OptF}$ si può anche esprimere come 

$$\mathrm{OptF} =  \frac{2}{n}\sum_{i=1}^{n}\mathbb{C}\mathrm{ov}(Y_i,\hat{f}(x_i))$$
Quindi più alta è la correlazione tra $Y_i$ e il suo valore previsto $\hat{f}(x_i)$, più elevato è l'ottimismo
...
* L'errore di previsione è dato da $\mathrm{ErrF} = \mathbb{E}(\mathrm{MSE}_{\mathrm{Te}}) = \mathbb{E}(\mathrm{MSE}_{\mathrm{Tr}}) + \mathrm{OptF}$

* L'ottimismo è un concetto importante perchè può essere utilizzato per ottenere una stima di $\mathrm{ErrF}$:
\begin{aligned}
\widehat{\mathrm{ErrF}} &= \mathrm{MSE}_{\mathrm{Tr}} + \mathrm{OptF}\\
\end{aligned}

* Per i modelli lineari, si può dimostrare che
...
* $\displaystyle \mathrm{OptF}=\frac{2\sigma^2p}{n}$ richiede la conoscenza di $\sigma^2$, che in pratica è un valore incognito
...
* Questo stimatore è noto come __Mallows' Cp__ : 
