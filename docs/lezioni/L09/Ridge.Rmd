...
$$\begin{aligned}
\mathbf{X}\mathbf{X}^\mathsf{T}(\mathbf{X}\mathbf{X}^\mathsf{T} + \lambda \mathbf{I} )^{-1} \mathbf{y}
\end{aligned}$$

Si noti che
* $\mathbf{X}^\mathsf{T}\mathbf{X}$ è $p\times p$, mentre la matrice di Gram $\mathbf{X}\mathbf{X}^\mathsf{T}$ è $n\times n$
...
* I valori previsti dalla regressione *ridge* dipendono solo da $\mathbf{K}=\mathbf{X}\mathbf{X}^\mathsf{T}$ dove $K_{ij}=\{\mathbf{K}\}_{ij} = x_i^\mathsf{T} x_j$
