...
```{r}
## codifico i valori mancanti come NA
is.na(X) <- (X==0)
test <- data.frame(y=y[test.id,], X[test.id,])
train <- data.frame(y=y[-test.id,], X[-test.id,])
...
yhat = numeric(m)
for (i in 1:B){
use = sample(n,n,rep=T)
tree = rpart (y~., data=train[use,], cp=0.0001)
yhat = yhat + predict(tree, newdata=test)
if (i%%10==0) cat(i, sqrt(mean ((yhat/i-test$y)^2)),"\n")
}
```
