...
Il grafico che segue rappresenta il diagramma di dispersione delle osservazioni del training set $(x_i,y_i)$ (pallini vuoti), e le mette a confronto con i valori previsti dal modello $(x_i,\hat{y}_i)$ (pallini pieni).
...
* Calcolare l'errore quadratico medio sui dati di training (*Training Mean Squared Error*)
$$\mathrm{MSE}_{\mathrm{Tr}} = \frac{1}{n}\sum_{i=1}^{n}(y_i - \hat{f}(x_i))^2$$ 
per la regressione polinomiale con $d=0,1,\ldots,29$.

* Si provi ad utilizzare il minor numero di righe di codice possibile.
...
* Le previsioni $\hat{y}^*_i$ ottenute con la regressione polinomiale utilizzando polinomi tradizionali (`raw = TRUE`) o ortogonali (`raw = FALSE`) sono le stesse? Perchè?

* Sul mio computer, se stimo la regressione polinomiale utilizzando i polinomi tradizionali (`raw = TRUE`)  ottengo alcuni `NA` nelle stime dei coefficienti per $d\geq 12$. Perchè?

* Se invece stimo la regressione polinomiale utilizzando i polinomi ortogonali (`raw = FALSE`) ottengo un messaggio di errore per $d\geq 24$. Perchè?
